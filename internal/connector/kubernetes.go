...
func (kc *KubeConnector) FindDeployments() error {
	...
	for _, deployment := range deploymentList.Items {
		...
		modelDeployment := &model.Deployment{
			...
			EdgeShare: 0, // TODO parse it from deployment's labels
		}
		// FIXME manual edge share:
		if strings.Contains(strings.ToLower(deploymentName), "a-app") {
			modelDeployment.EdgeShare = 0.5
		}
		if strings.Contains(strings.ToLower(deploymentName), "c") {
